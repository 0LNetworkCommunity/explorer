...
@Processor("base-historical")
export class BaseHistoricalProcessor extends WorkerHost {
  private readonly endpoint = "https://base.numia.xyz/v2/txs";
  ...
  public async triggerFetchHistoricalData() {
    await this.baseQueue.add("fetchHistoricalData", undefined);
  }
  ...
  private async fetchMintBurnEvents() {
    for (let page = 1; ; ++page) {
      ...
      for (const event of events) {
        ...
        const type =
          event.messageTypes[0] === "/base.tokenfactory.v1beta1.MsgMint"
            ? "mint"
            : "burn";
  ...
  private async fetchPoolSwapEvents() {
    const query = `
      SELECT
        ...
        parsed_amount_out,
        ingestion_timestamp
      FROM \`numia-data.base.base_swaps\`
      WHERE pool_id = '1721'
