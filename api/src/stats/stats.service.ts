...
export class StatsService {
  ...
  private async mapVersionsToTimestamps(versions: number[]): Promise<{ version: number; timestamp: number }[]> {
  ...
  private async getAccountsOnChainOverTime(): Promise<TimestampValue[]> {
    try {
      ...
      const allTimestampMappings = (await Promise.all(
  ...
  private async getAllWalletsBalances(
    ...
    try {
      ...
      const addressBalanceMap = new Map<string, { balance: number; timestamp: number }>();
      rows.forEach((row) => {
        const version = row.latest_version;
        const timestamp = versionToTimestampMap.get(version) ?? 0;
        addressBalanceMap.set(row.address, {
